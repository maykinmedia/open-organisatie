...
beheerinterface van Open Organisatie (hierna *de admin* genoemd).

In deze handleiding nemen we aan dat Open Organisatie geïnstalleerd is en beschikbaar
op het adres https://open-organisatie.gemeente.nl.
...
https://open-organisatie.gemeente.nl. Klik vervolgens op **Beheer**:
...
De *API's Open Organisatie* zijn niet toegankelijk zonder autorisatie.
Dit betekent dat elke applicatie die gegevens ophaalt of registreert in Open
Organisatie hiervoor geautoriseerd moet zijn. Dit kan via een token.
...
Het configuratiegedeelte dient om de Open Organisatie-installatie te configureren.
...
anders worden er door Open Organisatie geen notificaties verstuurd.
...
probleem in de Open Organisatie applicatie. Deze worden via de logs inzichtelijk
gemaakt.

Misc
----

**Websites** bevat gegevens over waar Open Organisatie gehost wordt. Zorg ervoor dat
...
2. Klik een onderwerp aan binnen een groep, bijvoorbeeld *Vestigingen*
...
Als voorbeeld zie je de lijstweergave van *Vestigingen*:

.. image:: assets/vestigingen_list.png
    :width: 100%
    :alt: Vestigingenlijst
...
Als voorbeeld zie je (een deel van) de detailweergave van een vestiging:

.. image:: assets/vestiging_detail.png
    :width: 100%
    :alt: Vestiging detail

1. De attributen van het vestiging worden opgelijst als bewerkbare velden. Sommige
