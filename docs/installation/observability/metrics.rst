...
Open Organisatie produces application metrics (using Open Telemetry).
...
.. important:: Some metrics are defined as "global scope".
   ...
   .. code-block:: promql
       ...
       avg by (type) (otel_openorganisatie_auth_user_count{scope="global"})
...
``openorganisatie.auth.user_count``
    ...
    .. code-block:: promql
        ...
        max by (type) (last_over_time(
          otel_openorganisatie_auth_user_count{scope="global"}
          [1m]
        ))

``openorganisatie.auth.login_failures``
...
``openorganisatie.auth.user_lockouts``
